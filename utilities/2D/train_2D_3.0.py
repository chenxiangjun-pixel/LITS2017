...
def main() -> None:
    ...
    train_ds = SliceDataset(os.path.join(para.training_set_2d_path, 'ct'),
    ...
    val_ds = SliceDataset(os.path.join(para.test_2d_path, 'ct'),
                          os.path.join(para.test_2d_path, 'seg'))
    val_dl = DataLoader(val_ds, para.batch_size, False, num_workers=para.num_workers,
                        pin_memory=para.pin_memory)
    ...
    for epoch in range(para.Epoch):
        ...
        with torch.no_grad():
            for ct, seg in val_dl:
                ct, seg = ct.cuda(), seg.cuda()
                outputs = model(ct)
                val_main_losses.append(loss_func(outputs[3], seg).item())
