...
class Metirc():
    ...
    def __init__(self, real_mask, pred_mask, voxel_spacing):
        ...
        if self.real_mask_surface_pts.shape[0] == 0 or self.pred_mask_surface_pts.shape[0] == 0:
            self.real2pred_nn = np.array([])
            self.pred2real_nn = np.array([])
        else:
            self.real2pred_nn = self.get_real2pred_nn()
            self.pred2real_nn = self.get_pred2real_nn()
    ...
    def get_surface(self, mask, voxel_spacing):
        ...
        surface_pts = np.array(list(zip(surface_pts[0], surface_pts[1], surface_pts[2]))).reshape(-1, 3)
    ...
    def get_RVD(self):
        ...
        real = float(self.real_mask.sum())
        if real == 0:
            return 0.0 if self.pred_mask.sum() == 0 else float('inf')
        return float(self.pred_mask.sum() - real) / real
    ...
    def get_ASSD(self):
        ...
        denom = self.real_mask_surface_pts.shape[0] + self.pred_mask_surface_pts.shape[0]
        if denom == 0:
            return 0.0
        return (self.pred2real_nn.sum() + self.real2pred_nn.sum()) / denom
    ...
    def get_RMSD(self):
        ...
        if denom == 0:
            return 0.0
        return math.sqrt((np.power(self.pred2real_nn, 2).sum() + np.power(self.real2pred_nn, 2).sum()) / denom)

    def get_MSD(self):
        """
        :return: 对称位置的最大表面距离 Maximum Symmetric Surface Distance
        """
        if self.pred2real_nn.size == 0 or self.real2pred_nn.size == 0:
            return 0.0
