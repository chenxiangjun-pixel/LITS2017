...
# 确保输出目录存在
if not os.path.exists(para.val_path):
    os.makedirs(para.val_path)
...
for file_index, file in enumerate(os.listdir(para.test_ct_path)):
    ...
    liver_metric = Metirc(seg_liver, liver_seg, ct.GetSpacing())
    tumor_metric = Metirc(seg_tumor, tumor_seg, ct.GetSpacing())
    ...
    pred_seg = sitk.GetImageFromArray(final_seg)

    pred_seg.SetDirection(ct.GetDirection())
    pred_seg.SetOrigin(ct.GetOrigin())
    pred_seg.SetSpacing(ct.GetSpacing())

    sitk.WriteImage(pred_seg, os.path.join(para.val_path, file.replace('volume', 'pred')))
...
writer = pd.ExcelWriter(r'D:\CXJ_code\Liver\LiTS_Tumor\result\3D_result\test\result.xlsx')
